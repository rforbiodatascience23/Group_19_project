...
# Joining since we did not need to group together two dataset. We won't use the joined dataset for the rest of the project, 
...
dat_clean <- dat_clean_02 |> 
   mutate(C_none = is.null(comorbidities))

#  mutate(C_none = ifelse(C_Dementia & C_Myocardial_infarction == 0, "Yes", "No"))
```


#mutate(Count_comorbidities = rowSums(select(starts_with("C_"))))) 
  #mutate(sum = sum(across(starts_with("C_"))))
  #mutate(Total = select(starts_with("C_"))) |> 
  #rowSums(na.rm = TRUE)
  #mutate(C_none = ifelse(rowSums(select(cirrhosis_data,starts_with("C_")) == 'Yes') > 0, 'No', 'Yes'))  
