...
dat_clean_02 <- dat_load_01 |> 
  ...
  # TIDYING = Create a column Comorbidities using the pivot_longer (one column / one line / one observation)
  pivot_longer(
    cols = starts_with("C_"), 
  ...
  # TIDYING = Create a column Etiology
  pivot_longer(
    cols = starts_with("E_"), 
    names_to = "Etiologies", 
  ...
  # Only select the patients that have comorbidity + the one that have C_none = Yes
  ...
  # Getting rid of observations with NA values
  drop_na(CPS) |> 
  drop_na(Sex) |> 
