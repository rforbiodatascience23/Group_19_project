...
The Child Pugh Score is a variable used to indicate the severity of a cirrhosis. In the first plot, we tried to see if there is a correlation between the Child Pugh Score and the Charlson Comorbidity Index, which predicts the mortality of patients suffering from comorbidities:
...
Charlson_vs_CPS_LV
```
On that plot, we can see that the relationship between CPS and Charlson CI can be totally different depending on the liver disease from which the patients suffer. There is no clear pattern that can be deducted from these graphs or from the plot of Charlson CI versus CPS. 
...
estimate |> 
  ...
  labs(
    title = "Ethiologies associated with CPS ",
    x = "Estimates(95% CIs)",
    y ="") +
...
In these plots, we can see that only one etiology is considered as having a significant correlation with CPS. This etiology is the Non-Alcoholic Fatty Liver Disease and a patient with this etiology has more chances of having a low CPS than a patient with another etiology. 
These results are also consistent with the results shown in the file '05_analysis1.qmd' because we saw in that file that nafld has a greater impact on mortality than alcohol or cryptogenic diseases. 
